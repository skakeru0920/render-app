...
- [ ] デプロイメントの準備をしましょう
  ...
  - [ ] ところで今、`src/db/knexfile.js` には 1 つの設定 (configuration) しかありませんが、これは良くありません。knexfile が本番環境で動作しているかどうかによって、異なる設定を返す必要があります。knexfile が設定される際、適切な設定になるようにするにはどうすればよいでしょう？
    `knexfile.js` を見ると、設定 (configuration) は JavaScript のオブジェクトになっているのに気が付くと思います。ということはローカル (development) と 本番 (production) の設定を分けるには、`develoment` 用と `production` 用の二つの configuration オブジェクトを作ればいいわけです。
    ```
    module.exports = {
      development: {
        client: "pg",
    ...
    ```
    このように二つのオブジェクトを作った上で、`db/index.js`に以下のコードを書いて、本番ではリモートの Web サービス上で設定された環境変数による設定が使われるようにしましょう。

    ```
    const environment = process.env.DATABASE_URL ? "production" : "development";
    knex(knexConfig[environment]);
    ```
    これでサーバーが走っている場所に応じて適切な設定が適用されるようになりました。ローカルの開発環境とリモートの Web サービスではサーバーからデータベースへの接続方法が違うのでこのような対応が必要です。
